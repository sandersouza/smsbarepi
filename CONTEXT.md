...
Ultima build: `BUILD_BID=202605012219-5477e75`, `TXTDBG BUILD:3C6B`, SHA1 `ed27d57d76b36bd9ec22824cbbc1a5cce3a46c8d` (`DEBUG=1`).
Audio SMS validado em link local com `third_party/smsplus/sound/sound.o` fornecendo `sound_init`, `sound_update`, `psg_write`, `fmunit_write`, `stream_update` e `snd`. O renderer SN76489 e local em `src/backend/sms/modules/smsplus/smsplus_bare_psg.c`, substituindo o objeto `third_party/smsplus/sound/sn76489.o` sem modificar `third_party`; YM2413/FM continua upstream. A ponte SMS usa mixer simples com ganho fixo 16x, `BoostNoise` preservado, clipping 16-bit, ring buffer mono, ganho final SMS em 100% e auto-gain SMS desativado. O kernel seleciona saida Circle por `SD:/cmdline.txt`: `sounddev=sndpwm` usa A/V analogico PWM; `sndhdmi` e `sndvchiq soundopt=2` forcam HDMI; `sndvchiq soundopt=0` tenta HDMI e cai para A/V PWM se HDMI nao tiver audio. `Settings -> Audio Gain` escala a saida selecionada. FM/YM2413 e controlado por `Settings -> FM Sound [ON/OFF]`, persistido como `fm_music=0|1`; `ON` ativa `sms.use_fm` e o mapa SMSJ antes de `system_init/system_reinit` para expor as portas Mark III/SMSJ `0xF0/0xF1/0xF2`; `OFF` volta ao mapa SMS sem FM. O latch `0xF2` e adaptado via wrapper local para retornar apenas bit 0, inclusive durante deteccao com I/O chip desabilitado.
UART/logger minimo inicia em `BT0`, antes de `screen` e `audio`; em `DEBUG=1`, `BT11` apenas anexa runtime UART detalhado/overlay ao canal ja ativo.
