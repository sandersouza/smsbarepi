...
- Viewport SMS usa framebuffer Circle fixo em 4:3 (`640x480`), base 256x192 para escala nativa e base fullscreen 256x243 para preencher a area visivel sem cortar. Presets de `Scale`: `1x` nativo, `2x` ponto medio entre nativo e fullscreen, `3x` fullscreen. O fullscreen e calculado como o maior multiplo de 1/16x que cabe em largura e altura; no modo fixo 4:3 o viewport esperado e ~`496x470`.
- `Scale` so fica ativo/visivel em `DEBUG=1`; em `DEBUG=0`, o runtime ignora `scale=` persistido, normaliza para fullscreen maximo e salva `scale=3`. O Settings nao exibe controle de proporcao.
...
- `sms.cfg` agora e lido/gravado somente via FatFs em `SD:/sms.cfg`; o espelho raw foi removido. A proporcao nao e configuravel: `proportion=` e `resolution=` de configs antigas sao ignorados, e o kernel aplica sempre `CComboScreenDevice::Resize(640, 480)` no boot sem ler nem gravar `SD:/config.txt`.
...
- `build/src/sms.img`
- Ultima build local: `BUILD_BID=202604301842-2fed053`, `TXTDBG BUILD:9FEC`, SHA1 `df9db1f47ad1be5ce41bcd7b1a3a9be21553a7d3` (`DEBUG=0`).
